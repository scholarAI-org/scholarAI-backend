...
class AdminAuditLoggingActionsTests(unittest.TestCase):
    def test_audit_failure_rolls_back_scholarship_and_notification(self):
        with self.session_factory() as db:
            item = db.get(Scholarship, self.scholarship_id)
            item.source_url = "https://example.com/source"
            item.apply_link = "https://example.com/apply"
            item.image_url = "https://example.com/image.jpg"
            item.no_deadline = True
            db.commit()
            original_title = item.title
            original_count = db.query(Scholarship).count()
        cases = (
            (
                "post",
                "/api/scholarships/",
                {"source": "for9a", "source_id": "atomic-test", "title": "Atomic"},
                "app.api.scholarships",
            ),
            (
                "put",
            ...
            (
                "post",
                f"/admin/scholarships/{self.scholarship_id}/approve",
                {},
                "app.api.admin",
            ),
            (
        ...
        for method, path, body, module in cases:
            with self.subTest(method=method, path=path):
                with patch(
                    f"{module}.create_audit_log",
                ...
                with self.session_factory() as db:
                    self.assertEqual(db.query(Scholarship).count(), original_count)
                    item = db.get(Scholarship, self.scholarship_id)
